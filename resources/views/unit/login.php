...
<body class="min-h-screen flex items-center justify-center p-4 transition-colors duration-300" 
  ...
  <div class="w-full max-w-md relative z-10">
    ...
    <div class="text-center mb-8">
      ...
      <h1 class="text-3xl font-bold mb-2 transition-colors" :class="isDark ? 'text-white' : 'text-slate-900'">单位端登录</h1>
      <p class="transition-colors" :class="isDark ? 'text-slate-400' : 'text-slate-600'">欢迎回来，请登录您的账号</p>
    </div>
    ...
    <form id="login-form" class="relative overflow-hidden rounded-2xl backdrop-blur-xl border p-8 shadow-2xl transition-colors"
      ...
      <div class="relative z-10 space-y-5">
        <div>
          <label class="block text-sm font-medium mb-2 transition-colors" :class="isDark ? 'text-slate-300' : 'text-slate-700'">账号</label>
          <div class="relative">
  ...
  <script>
    // 检查是否已登录（使用单位端专用 token 键，与管理端分离）
    if (localStorage.getItem('unit_token')) {
    ...
    document.getElementById('login-form').onsubmit = async (e) => {
      ...
      try {
        const res = await fetch('/unit/auth/login', {
          method: 'POST',
